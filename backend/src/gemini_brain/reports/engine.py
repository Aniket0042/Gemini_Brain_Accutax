...
These reports are the fallback when a matching Nest `/report/*` API is
unavailable. Segmented P&L, invoice-level aging, contact statements, and VAT
detail each have a REST counterpart; `_retrieve()` calls HTTP first and only
dispatches here when that call fails. Without this fallback those questions
miss the router entirely and fall to the NL-to-SQL tier, which costs 5.9s p50
and 22.3s p95 and asks a model to invent SQL that is already written here.
