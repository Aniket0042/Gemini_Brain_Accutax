...
# ─────────────────────────────────────────────────────────────
# Conversation working memory (sliding window + rolling summary)
# ─────────────────────────────────────────────────────────────
#: Last N messages (~6 turns) injected into the model on each query.
MEMORY_WINDOW_MESSAGES: int = 12
#: Truncate stored assistant turns in the window so prompts stay cheap.
MEMORY_ASSISTANT_TRUNCATE_CHARS: int = 400
#: Start compacting older turns into conversation_state.summary past this count.
#: Keep this at or below MEMORY_WINDOW_MESSAGES so the first turn is never
#: dropped from the prompt without already being folded into the summary.
MEMORY_SUMMARY_TRIGGER_MESSAGES: int = 12
#: Hard cap on the CONVERSATION SO FAR block appended to system prompts.
...
- Retrieved records, uploaded documents, and internal tool output are background — never quote SQL, schema, or raw dumps, and never name those sources.
- This chat's prior user and assistant messages ARE the conversation. If the user asks what they asked before, recap those turns. Never claim you have no conversation history when prior messages or CONVERSATION SO FAR are present.
- Never invent a follow-up question from the user. Never write a line that starts with User: or Assistant: in your answer."""
