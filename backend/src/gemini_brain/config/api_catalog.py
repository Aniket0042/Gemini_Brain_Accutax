...
def get_api_catalog() -> str:
    """Prefer the live Accutax OpenAPI catalog; fall back to the static snapshot."""
    try:
        from gemini_brain.config.accutax_openapi import catalog_text
        live = catalog_text()
        if live:
            return live
    except Exception:
...
GET /accounting/journal-entries
  query: userId* (camelCase, REQUIRED), organizationId* (camelCase, REQUIRED), startDate, endDate (YYYY-MM-DD), page=1, limit=50
  → Journal entries for the date range (invoices, payments, COGS, manual). Filter by startDate/endDate — "posted today" is startDate=endDate=today.
...
GET /report/profit-loss
  ...
  Prefer this over /income/total whenever the user wants a report, chart, or breakdown.
