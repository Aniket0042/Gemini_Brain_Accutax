...
from gemini_brain.router.rules import get_fast_router_rules, redirect_for_aging
from gemini_brain.router.rules import PERIOD_REPORT_ENDPOINTS, RANKED_REPORT_ENDPOINTS
from gemini_brain.utils.ranking import extract_direction_from_text, extract_requested_count
...
def _build_params_for_endpoint(
    ...
    if endpoint == "/expense/total":
    ...
    elif endpoint in PERIOD_REPORT_ENDPOINTS:
        params = {
            "organization_id": organization_id,
            "start_date": window.date_from.isoformat(),
            "end_date": window.date_to.isoformat(),
        }
        if endpoint in RANKED_REPORT_ENDPOINTS:
            params["limit"] = extract_requested_count(clean_q, default=20)
            params["sort_order"] = "asc" if extract_direction_from_text(clean_q) else "desc"
        ...
        params = {
            "organization_id": organization_id,
            "months": dates.forecast_months(clean_q),
            "start_date": window.date_from.isoformat(),
            "end_date": window.date_to.isoformat(),
        }
    ...
    elif endpoint == "/dashboard/web":
        params = {
            "user_id": str(uid),
            "organization_id": organization_id,
            "year": str(window.date_from.year),
            "start_date": window.date_from.isoformat(),
            "end_date": window.date_to.isoformat(),
        }
    elif endpoint in ("/bank/manual/accounts", "/bank/transactions/uncategorized"):
        params = {
            "organization_id": organization_id,
        }
        ...
        params = {
            "userId": int(uid) if str(uid).isdigit() else DEFAULT_USER_ID,
            "limit": 20,
            "start_date": window.date_from.isoformat(),
            "end_date": window.date_to.isoformat(),
        }
    ...
    elif endpoint in ("/expense/list", "/expense/list_filter"):
        status = "unpaid" if "unpaid" in clean_q.lower() else ("paid" if "paid" in clean_q.lower() else "all")
        params = {
            "userId": int(uid) if str(uid).isdigit() else DEFAULT_USER_ID,
            "limit": 20,
        ...
        if "price" in clean_q.lower():
            ...
            params["order"] = "desc"
    elif endpoint == "/accounting/journal-entries":
        start = window.date_from.isoformat()
        end = window.date_to.isoformat()
        params = {
            "userId": int(uid) if str(uid).isdigit() else uid,
            "organizationId": organization_id,
            "startDate": start,
            "endDate": end,
            # snake_case kept so window widening can see the range even after
            # OpenAPI drops unknown aliases.
            "start_date": start,
            "end_date": end,
            "limit": 50,
        }
...
def fast_route(
    ...
    for pattern, rule_name, endpoint, intent in FAST_ROUTER_RULES:
        if pattern.search(clean_q):
            aging_target = redirect_for_aging(rule_name, clean_q)
            if aging_target:
                redirected = next((r for r in FAST_ROUTER_RULES if r[1] == aging_target), None)
                if redirected is None:
                    logger.info(
                        "FastRouter DEFER: '%s' cannot express an ageing qualifier "
                        "and '%s' is unavailable", rule_name, aging_target,
                    )
                    return None
                logger.info("FastRouter REDIRECT: '%s' → '%s'", rule_name, aging_target)
                _, rule_name, endpoint, intent = redirected
