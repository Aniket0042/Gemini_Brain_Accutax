...
from gemini_brain.config.constants import DEFAULT_USER_ID, GEMINI_MODEL
...
Choose exactly one tool from the provided tools that best answers the question. Never answer from your own knowledge when a tool exists.
...
def route_with_gemini(
    ...
    from gemini_brain.tools.chat_index import declarations_for, resolve_tool

    names = retrieve_tool_names(
        query,
        session_state=ctx.session_state if ctx else None,
    )
    declarations = declarations_for(names, flavor="bedrock")
    ...
    if ctx and ctx.session_state:
        ...
        system_prompt = append_memory_block(system_prompt, ctx.session_state, for_routing=True)
...
def select_endpoint_structured(
    ...
    """
    from gemini_brain.tools.chat_index import resolve_tool

    ctx = RequestCtx(
        org_id=org_id,
        user_id=int(user_id) if str(user_id).isdigit() else DEFAULT_USER_ID,
    ...
    if tool_res.name == "unsupported":
        return None, tool_res.tokens_in, tool_res.tokens_out

    spec = resolve_tool(tool_res.name)
    if spec is None:
        return None, tool_res.tokens_in, tool_res.tokens_out
    try:
