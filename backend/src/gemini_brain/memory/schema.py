...
def initialize_tables(db_name: str = "") -> None:
    ...
    try:
        ...
        cur.execute("""
            SELECT EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_name='model_arena_chat_sessions'
        ...
        cur.execute("""
            SELECT EXISTS (
                SELECT 1 FROM information_schema.columns
                WHERE table_name='model_arena_chat_messages'
                  AND column_name='blocks'
                  AND table_schema='public'
            )
        """)
        has_blocks = cur.fetchone()[0]
        if not has_blocks:
            cur.execute(
                "ALTER TABLE public.model_arena_chat_messages ADD COLUMN blocks JSONB;"
            )

        conn.commit()
        logger.debug("Database tables initialized successfully for %s", db_name)
