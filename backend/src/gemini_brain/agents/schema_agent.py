...
def handle(task: str, params: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    Every task above except ``get_sample_values`` only ever touches static
    metadata (information_schema, the hardcoded ENTITY_MAP/KNOWN_JOINS) — no
    tenant row data, so no organization_id is needed. ``get_sample_values``
    reads real column values and must be tenant-scoped; ``organization_id``
    is forwarded to it from ``params`` for that reason.
    """
    params = params or {}
    ...
    if task == "get_tables":
        ...
        return _task_get_sample_values(
            params.get("table", ""),
            params.get("column", ""),
            params.get("limit", 20),
...
def _task_get_sample_values(
    table: str, column: str, limit: int = 20, organization_id: Optional[int] = None
) -> Dict:
    table = table.lower().strip()
    column = column.lower().strip()
    schema = get_schema()
    ...
    # Tenant isolation: a table carrying organization_id holds real customer
    # data (emails, names, amounts, ...) — sampling it without a verified org
    # would return values across every tenant. Reject rather than silently
    # running unscoped; tables with no organization_id (shared reference
    # tables like status_type/currency_type) have no tenant boundary to
    # enforce and are sampled as before.
    conditions = []
    if schema[table]["has_organization_id"]:
        if organization_id is None:
            return {
                "error": (
                    f"organization_id is required to sample values from tenant "
                    f"table '{table}'."
                )
            }
        conditions.append(f"organization_id = {int(organization_id)}")
    if schema[table]["has_is_deleted"]:
        conditions.append("is_deleted = false")
    where_clause = f" WHERE {' AND '.join(conditions)}" if conditions else ""

    try:
        cols, rows = execute_sql(
            f"SELECT DISTINCT {column} FROM {table}{where_clause} ORDER BY {column} LIMIT {int(limit)}",
