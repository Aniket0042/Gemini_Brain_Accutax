...
from gemini_brain.sql_fallback.tenant_sql_guard import (
    enforce_tenant_isolation_sql as _enforce_tenant_isolation_sql,
)
...
# Organization for the in-flight task, published to Postgres as app.current_org
# so RLS applies to every query this module runs. Set once in handle().
_active_org: ContextVar[Optional[int]] = ContextVar("finance_active_org", default=None)


# ──────────────────────────────────────────────
# SQL literal quoting
# ──────────────────────────────────────────────
# Task params reach these builders from model tool-calls, which in turn derive
# from user text. Interpolating them into SQL directly is injectable, so every
# value goes through psycopg2's own adapter rather than an f-string quote.

def _lit(value: Any) -> str:
    """Return ``value`` as a properly escaped SQL literal."""
    ...
    if isinstance(value, (int, float, Decimal)):
        return str(value)
    return adapt(str(value)).getquoted().decode("utf-8")


def _like(value: Any, *, prefix: str = "%", suffix: str = "%") -> str:
    """Return a escaped ``LIKE`` pattern literal wrapping ``value``.

    LIKE metacharacters in the value itself are escaped so a user searching for
    a literal '%' does not match everything.
    """
    text = str(value if value is not None else "")
    escaped = text.replace("\\", "\\\\").replace("%", r"\%").replace("_", r"\_")
    return _lit(f"{prefix}{escaped}{suffix}")


# ──────────────────────────────────────────────
# Tenant isolation safety net for free-form SQL
# ──────────────────────────────────────────────
# `_enforce_tenant_isolation_sql` (imported above from tenant_sql_guard.py,
# shared with sql_fallback/sql_engine.py) is a defense-in-depth check on top
# of the structured tasks, which already inject organization_id safely via
# _maybe_org/_org_frag: it force-corrects any organization_id literal in
# LLM-generated SQL to match the verified session org_id, and injects a
# filter if a tenant table is queried with no org filter at all.
...
def _run_sql(sql: str) -> Dict[str, Any]:
    ...
    The organization is read from ``_active_org`` rather than threaded through
    all 50 call sites, so every query in this module runs under RLS.
    """
    try:
        cols, rows = execute_sql(sql, org_id=_active_org.get())
        total_rows = len(rows)
...
def handle(task: str, params: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if org_raw is not None:
            org_token = _active_org.set(int(org_raw))
...
def _build_where(filters: Dict, entity: str = "income", params: Dict = None) -> tuple:
    ...
    if filters.get("customer"):
        ...
        clauses.append(f"LOWER(c.name) LIKE {_like(filters['customer'].lower())}")
    elif filters.get("vendor"):
        joins.append(f"JOIN contacts c ON c.id = {alias}.contact_id")
        clauses.append(f"LOWER(c.name) LIKE {_like(filters['vendor'].lower())}")

    if filters.get("invoice_number"):
        clauses.append(f"{alias}.invoice_number = {_lit(filters['invoice_number'])}")

    if filters.get("status"):
        joins.append(f"LEFT JOIN status_type st ON st.id = {alias}.status_type_id")
        status_val = filters["status"]
        if isinstance(status_val, list):
            status_list = ", ".join(_lit(s.lower()) for s in status_val)
            clauses.append(f"LOWER(st.value) IN ({status_list})")
        else:
            clauses.append(f"LOWER(st.value) = {_lit(status_val.lower())}")

    if filters.get("date_from"):
        date_col = "invoice_date" if entity == "income" else "reception_date"
        clauses.append(f"CAST({alias}.{date_col} AS DATE) >= {_lit(filters['date_from'])}")
    if filters.get("date_to"):
        date_col = "invoice_date" if entity == "income" else "reception_date"
        clauses.append(f"CAST({alias}.{date_col} AS DATE) <= {_lit(filters['date_to'])}")
...
def _maybe_org(clauses: list, alias: str, params: Dict):
    """Add organization_id filter to clauses list.

    Fails closed: every caller of this helper queries a tenant table, so a
    missing organization_id must reject the query rather than silently
    return unfiltered (cross-tenant) rows. Callers run inside `handle()`'s
    dispatch try/except (see below), so raising here becomes a clean
    {"error": ...} response, not a crash.
    """
    org_id = params.get("organization_id")
    if org_id is None:
        raise ValueError("organization_id is required for this query.")
    clauses.append(f"{alias}.organization_id = '{int(org_id)}'")


def _org_frag(alias: str, params: Dict) -> str:
    """Return '\n  AND alias.organization_id = N' fragment.

    Fails closed like `_maybe_org` above — see its docstring.
    """
    org_id = params.get("organization_id")
    if org_id is None:
        raise ValueError("organization_id is required for this query.")
...
def _task_top_customers(params: Dict) -> Dict:
    ...
    if filters.get("date_from"):
        clauses.append(f"CAST(inc.invoice_date AS DATE) >= {_lit(filters['date_from'])}")
    if filters.get("date_to"):
        clauses.append(f"CAST(inc.invoice_date AS DATE) <= {_lit(filters['date_to'])}")
...
def _task_invoice_details(params: Dict) -> Dict:
    ...
    # An invoice number is only unique within a tenant, and this row exposes the
    # customer's name, email and phone — so it is scoped like every other lookup.
    org_id = params.get("organization_id")
    if org_id is None:
        return {"error": "organization_id is required", "results": [], "row_count": 0}
...
WHERE inc.invoice_number = {_lit(inv_num)}
  AND inc.organization_id = {int(org_id)}
ORDER BY i.name"""
    result = _run_sql(sql)
    if result["success"] and result["results"]:
...
def _task_general_ledger(params: Dict) -> Dict:
    ...
    if account_name:
        clauses.append(f"LOWER(jel.account_name) LIKE {_like(account_name.lower())}")
    if account_code:
        clauses.append(f"jel.account_code = {_lit(account_code)}")
...
def _task_chart_of_accounts(params: Dict) -> Dict:
    ...
    clauses = []
    if account_type:
        clauses.append(f"LOWER(coa.account_type) = {_lit(account_type.lower())}")
...
def _task_journal_entry_search(params: Dict) -> Dict:
    ...
    if params.get("reference"):
        clauses.append(f"LOWER(je.reference_number) LIKE {_like(params['reference'].lower())}")
    if params.get("notes") or params.get("description"):
        term = params.get("notes") or params.get("description")
        clauses.append(f"LOWER(je.description) LIKE {_like(term.lower())}")
...
def _task_bank_transactions(params: Dict) -> Dict:
    ...
    if params.get("bank_account"):
        clauses.append(f"LOWER(bt.account_name) LIKE {_like(params['bank_account'].lower())}")
    if params.get("category"):
        clauses.append(f"LOWER(bt.category) LIKE {_like(params['category'].lower())}")
...
def _check_table_exists(table_name: str) -> bool:
    ...
    try:
        result = _run_sql(f"SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name={_lit(table_name)}")
...
def _task_inventory_status(params: Dict) -> Dict:
    ...
    if params.get("item_name"):
        clauses.append(f"LOWER(itm.name) LIKE {_like(params['item_name'].lower())}")
...
def _task_inventory_movements(params: Dict) -> Dict:
    ...
    if params.get("movement_type"):
        clauses.append(f"LOWER(im.movement_type) LIKE {_like(params['movement_type'].lower())}")
...
def _task_audit_trail(params: Dict) -> Dict:
    ...
    if params.get("entity_type") or params.get("transaction_type"):
        ...
        clauses.append(f"LOWER(CAST(atr.transaction_type AS TEXT)) LIKE {_like(et.lower())}")
    ...
    if params.get("action") or params.get("action_type"):
        ...
        clauses.append(f"LOWER(CAST(atr.action_type AS TEXT)) LIKE {_like(act.lower())}")
    if params.get("user") or params.get("user_name"):
        u = params.get("user") or params.get("user_name")
        clauses.append(f"LOWER(atr.user_name) LIKE {_like(u.lower())}")
    if params.get("transaction_number"):
        clauses.append(f"LOWER(atr.transaction_number) LIKE {_like(params['transaction_number'].lower())}")
...
def _task_audit_activity(params: Dict) -> Dict:
    ...
    if params.get("method") or params.get("http_method"):
        ...
        clauses.append(f"UPPER(al.http_method) = {_lit(m.upper())}")
    if params.get("url"):
        clauses.append(f"al.url LIKE {_like(params['url'])}")
    if params.get("user_id"):
        clauses.append(f"al.user_id = {int(params['user_id'])}")
    if params.get("entity_type"):
        clauses.append(f"LOWER(CAST(al.entity_type AS TEXT)) LIKE {_like(params['entity_type'].lower())}")
...
def _task_project_profitability(params: Dict) -> Dict:
    ...
    if params.get("project_name"):
        clauses.append(f"LOWER(prj.project_name) LIKE {_like(params['project_name'].lower())}")
...
def _task_customer_payments(params: Dict) -> Dict:
    ...
    if params.get("customer"):
        clauses.append(f"LOWER(c.name) LIKE {_like(params['customer'].lower())}")
    if params.get("status"):
        clauses.append(f"LOWER(cp.status) = {_lit(params['status'].lower())}")
    if filters.get("year"):
...
def _task_vendor_payments(params: Dict) -> Dict:
    ...
    if params.get("vendor"):
        clauses.append(f"LOWER(c.name) LIKE {_like(params['vendor'].lower())}")
    if filters.get("status"):
        clauses.append(f"LOWER(st.value) = {_lit(filters['status'].lower())}")
    if filters.get("year"):
        clauses.append(f"EXTRACT(YEAR FROM CAST(e.reception_date AS DATE)) = {_safe_year(filters['year'])}")
...
WHERE COALESCE(st.value, '') NOT IN ('PAID', 'CANCELLED', 'VOIDED')
  ...
  AND CAST(inc.due_date AS DATE) BETWEEN CURRENT_DATE - INTERVAL '30 days' AND CURRENT_DATE + INTERVAL '{int(days)} days'{_org_frag("inc", params)}
