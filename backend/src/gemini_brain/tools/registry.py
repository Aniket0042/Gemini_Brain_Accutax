...
REGISTRY: Dict[str, ToolSpec] = {
    "profit_loss": ToolSpec(
        ...
        description=(
            ...
            "'income report', 'revenue chart', 'graph of income', 'monthly revenue'. "
    ...
    "income_total": ToolSpec(
        ...
        description=(
            ...
            "Do NOT use for P&L reports, income statements, or charts of monthly revenue "
            "(use profit_loss / /report/profit-loss)."
        ),
    ...
    "journal_entries": ToolSpec(
        ...
        description="List journal entries (invoices, payments, COGS, manual) for a date range. Use period='today' when the user asks what was posted or recorded today. Include every source type, not only payments.",
    ...
    # ── Report APIs (REST first; SQL rpt_ only if the API is unavailable) ──
    "aged_receivables_detail": ToolSpec(
        ...
        handler=make_api_handler("/report/invoice-details"),
        formatter="row_table",
        intent=3,
    ),
    "aged_payables_detail": ToolSpec(
        ...
        endpoint="/report/aged-payables-detail",
        params=ReportAsOfLimitParams,
        handler=make_api_handler("/report/aged-payables-detail"),
        formatter="aging_buckets",
        intent=3,
    ),
    "bills_by_contact": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/bills-by-contact"),
        formatter="row_table",
        intent=3,
    ),
    "expenses_by_contact": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/expenses-by-contact"),
        formatter="row_table",
        intent=3,
    ),
    "supplier_statement": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/supplier-statement-of-account"),
        formatter="row_table",
        intent=3,
    ),
    "profit_loss_by_project": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/profit-loss-by-project"),
        formatter="row_table",
        intent=3,
    ),
    "profit_loss_by_cost_center": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/profit-loss-by-cost-center"),
        formatter="row_table",
        intent=3,
    ),
    "profit_loss_by_branch": ToolSpec(
        name="profit_loss_by_branch",
        description=(
            "Revenue segmented by branch. "
            "Use for: 'P&L by branch', 'revenue per branch'."
        ),
        endpoint="/report/profit-loss-by-branch",
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/profit-loss-by-branch"),
        formatter="row_table",
        intent=3,
    ),
    "sales_by_project": ToolSpec(
        ...
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/sales-by-project"),
        formatter="row_table",
        intent=3,
    ),
    "sales_by_branch": ToolSpec(
        name="sales_by_branch",
        description=(
            "Invoice count and total revenue per branch. "
            "Use for: 'sales by branch', 'revenue per branch'."
        ),
        endpoint="/report/sales-by-branch",
        params=ReportPeriodLimitParams,
        handler=make_api_handler("/report/sales-by-branch"),
        formatter="row_table",
        intent=3,
    ),
    "estimate_conversion": ToolSpec(
        ...
        params=ReportPeriodParams,
        handler=make_api_handler("/report/estimate-conversion-rate"),
        formatter="row_table",
        intent=3,
    ),
    "vat_input_output": ToolSpec(
        ...
        params=ReportPeriodParams,
        handler=make_api_handler("/report/vat-input-output"),
        formatter="row_table",
        intent=3,
    ),
    "vat_export_return": ToolSpec(
        ...
        endpoint="/report/export-return",
        params=ReportPeriodParams,
        handler=make_api_handler("/report/export-return"),
        formatter="kv_summary",
        intent=3,
    ),
...
def resolve_tool(name: str) -> Optional[ToolSpec]:
    """Look up a tool by name in the live chat index, then REGISTRY."""
    try:
        from gemini_brain.tools.chat_index import resolve_tool as indexed
        found = indexed(name)
...
def tool_spec_for_endpoint(endpoint: Optional[str]) -> Optional[ToolSpec]:
    if not endpoint:
        return None
    for spec in REGISTRY.values():
        if spec.endpoint == endpoint:
            return spec
    try:
        from gemini_brain.tools.chat_index import tool_by_endpoint
        return tool_by_endpoint(endpoint)
    except Exception:
        return None


def gemini_declarations(names: Optional[Sequence[str]] = None) -> List[Dict[str, Any]]:
    """Build Gemini function declaration dictionaries.

    With no `names`, returns the full REGISTRY (tests / eval harness).
    Pass `names` to emit a retrieved subset, including OpenAPI-only tools.
    """
    if names is not None:
        from gemini_brain.tools.chat_index import declarations_for
        return declarations_for(names, flavor="gemini")
    decls = []
    for spec in REGISTRY.values():
...
def bedrock_declarations(names: Optional[Sequence[str]] = None) -> List[Dict[str, Any]]:
    """Build AWS Bedrock toolSpec declarations.

    Pass `names` for the retrieved subset used by the live router.
    """
    if names is not None:
        from gemini_brain.tools.chat_index import declarations_for
        return declarations_for(names, flavor="bedrock")
    decls = []
    for spec in REGISTRY.values():
        schema = spec.params.model_json_schema()
        props = {}
