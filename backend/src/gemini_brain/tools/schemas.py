...
class JournalEntriesParams(BaseModel):
    ...
    period: str = Field(
        default="this year",
        description="Time period (e.g. 'today', 'yesterday', 'this month', 'this year').",
    )
    limit: int = Field(default=50, ge=1, le=100)

    def to_query(self, ctx: RequestCtx) -> Dict[str, Any]:
        w = dates.resolve(self.period)
        start = w.date_from.isoformat()
        end = w.date_to.isoformat()
        return {
            "userId": ctx.user_id,
...
class ReportAsOfParams(BaseModel):
    ...
    def to_query(self, ctx: RequestCtx) -> Dict[str, Any]:
        # Clamp to today: dates.resolve() used to lack a "today" phrase and
        # fell through to "this year" (31 December). Aging measured against a
        # future date inflates days_overdue. Keep the clamp even though
        # resolve("today") is now a real single-day window.
