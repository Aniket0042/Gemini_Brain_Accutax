...
days", or the default. If they asked about Q1 2025, or 2024, last month, or a
single day ("today", "yesterday"), that is a deliberate choice: silently
reporting a different period's numbers would be worse than saying there is
nothing there.
...
#: A one-day window ("today", "yesterday") is as specific as "last month".
#: Widening it silently replaces the asked-for day with months of history.
MIN_WIDEN_SPAN_DAYS: int = 2
...
def plan_widenings(
    ...
    *,
    ...
    params, unparseable dates, a single-day ask, or a window that ends in the
    past (a deliberate historical period, which must not be silently replaced).
    """
    if not isinstance(query_params, dict):
        return []

    start = _parse_iso(query_params.get("start_date") or query_params.get("startDate"))
    end = _parse_iso(query_params.get("end_date") or query_params.get("endDate"))
    ...
    if current_span < MIN_WIDEN_SPAN_DAYS:
        logger.debug("Not widening: %s to %s is a single-day ask", start, end)
        return []
...
def widened_params(query_params: Dict[str, Any], start: str, end: str) -> Dict[str, Any]:
    ...
    if "startDate" in widened:
        widened["startDate"] = start
    if "endDate" in widened:
        widened["endDate"] = end
    return widened
