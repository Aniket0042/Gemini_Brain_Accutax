...
# ── Module-level helpers used by the agents/ SQL pipeline ────────────────────
# These share the same boto3 singleton / backoff / extract helpers above so
# agents no longer need a second Bedrock client implementation.

MODEL_ID = settings.bedrock_model_id
MODEL_ID_FAST = settings.bedrock_model_id_fast

_tl = threading.local()


def reset_token_usage() -> None:
    """Reset per-request token counters (call at the start of every agent run)."""
    _tl.total_input = 0
    _tl.total_output = 0
    _tl.calls = 0
    _tl.model_tokens = {}
...
def add_token_usage(input_tokens: int, output_tokens: int, model_id: str = "") -> None:
...
def _compute_request_cost_usd() -> float:
    model_tokens: Dict[str, Dict[str, int]] = getattr(_tl, "model_tokens", {}) or {}
    if model_tokens:
        total = 0.0
        for mid, counts in model_tokens.items():
            rates = bedrock_price_for_model(mid)
            total += (counts["input"] / 1_000_000) * rates["input"]
            total += (counts["output"] / 1_000_000) * rates["output"]
        return round(total, 6)
    rates = bedrock_price_for_model(MODEL_ID)
    inp = getattr(_tl, "total_input", 0)
    out = getattr(_tl, "total_output", 0)
    return round((inp / 1_000_000) * rates["input"] + (out / 1_000_000) * rates["output"], 6)


def get_request_token_usage() -> Dict[str, Any]:
    """Aggregate token usage for the current agent request (thread-local)."""
    return {
        "input_tokens": getattr(_tl, "total_input", 0),
        "output_tokens": getattr(_tl, "total_output", 0),
        "llm_calls": getattr(_tl, "calls", 0),
        "cost_usd": _compute_request_cost_usd(),
    }


# Back-compat alias used by agents/api_agent.py
...
converse_with_tools = converse_tools
