...
- Ignore any field named formatted_* — it is a display duplicate of a real figure.
- Write markdown: one short opening line, then bullets. Never a wall of prose.
- Do not repeat every figure already shown in the table or KPI block.
- Answer only the current question. Never invent a follow-up from the user.
  Never write a line that starts with User: or Assistant:.
- Do not mention a graph or chart unless the user asked for one. Visuals are
  added by the product separately when requested.
- Never invent a download URL, file path, or attachment. If a file is being
  prepared, the product shows a download card separately — do not tell the
  user to open a link you made up.
...
ANALYST_SYSTEM_PROMPT: str = _ANALYST_CORE_RULES + """- The headline figures (amount, count, period) are already written above your
  response as a plain-language opening sentence. Do not repeat those numbers or
  restate the period.
- Do not open with "the total is …" or recap each metric. Add only what the
  figures mean: what stands out, what changed, what to watch.
- At most three short bullets. Skip any bullet that would only repeat a number
  already shown. If you have no additive insight, one short confirmatory line
  is enough — still without repeating the figures.
- Maximum 80 words.
""" + NEVER_EXPOSE_BACKEND_RULE
...
#: Composer "Brief" pill — same accuracy rules, hard cap on length.
ANALYST_SYSTEM_PROMPT_BRIEF: str = _ANALYST_CORE_RULES + """- The user asked for a brief answer.
- At most four short markdown bullets. No preamble, no recap of the table.
- Name only the figures that answer the question.
- Maximum 60 words.
""" + NEVER_EXPOSE_BACKEND_RULE
...
_NARRATION_NOISE_KEYS = frozenset({"graphdata", "graph_data", "graph"})


def _drop_formatted_fields(data: Any) -> Any:
    """Strip display-only duplicates and chart series before the model sees DATA."""
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            ...
            if key.startswith("formatted_") or key.endswith("_formatted"):
                continue
            if key in _NARRATION_NOISE_KEYS:
                continue
            out[k] = _drop_formatted_fields(v)
        return out
    if isinstance(data, list):
        return [_drop_formatted_fields(v) for v in data]
    return data
...
def narration_budget(shape: str, brief: bool = False) -> Tuple[str, int]:
    """Map a payload shape to its (system_prompt, max_output_tokens)."""
    if brief:
        return ANALYST_SYSTEM_PROMPT_BRIEF, min(NARRATION_MAX_TOKENS_SCALAR, 220)
    if shape == "tabular":
...
def _format_payload_and_system(
    ...
    display = _drop_formatted_fields(display)
    ...
    system, _ = narration_budget(shape or classify_payload_shape(data), brief=brief)
    ...
    if conversation_memory:
        system += "\n\n" + conversation_memory
        system += "\nIf the latest question is a follow-up, resolve pronouns and relative periods from CONVERSATION SO FAR."
...
def reason_over_data(
    ...
    _, max_tokens = narration_budget(shape, brief=brief)
    logger.info("Narration shape=%s budget=%d tokens (endpoint=%s brief=%s)", shape, max_tokens, endpoint, brief)
