...
def test_normalize_envelope_strips_invented_user_turn():
    from gemini_brain.resilience.output_guard import strip_invented_turns

    raw = (
        "Operating profit is AED 1,303,401.85.\n\n"
        "User: That's great, can you provide the monthly revenue vs expenses graph?\n\n"
        "Here is the graph."
    )
    assert strip_invented_turns(raw) == "Operating profit is AED 1,303,401.85."
    # Recapping a prior turn as the whole answer must not be wiped.
    recap = "User: Show cash flow"
    assert strip_invented_turns(recap) == recap

    norm = normalize_envelope({"answer": raw, "results": [{"net_profit": 1}]})
    assert "User:" not in norm["answer"]
    assert "Operating profit is AED 1,303,401.85." in norm["answer"]
