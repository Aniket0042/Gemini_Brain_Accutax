...
def test_param_model_injects_tenant_from_context():
    ...
    assert "startDate" in q_je
    assert "endDate" in q_je

    q_today = JournalEntriesParams(period="today", limit=10).to_query(ctx)
    from gemini_brain.router.dates import today as org_today
    day = org_today().isoformat()
    assert q_today["startDate"] == q_today["endDate"] == day
