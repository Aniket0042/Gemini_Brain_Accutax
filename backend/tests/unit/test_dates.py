...
def test_today_and_yesterday():
    anchor = datetime.date(2026, 9, 11)
    w = resolve("today", anchor=anchor)
    assert w.date_from == w.date_to == datetime.date(2026, 9, 11)

    w_y = resolve("yesterday", anchor=anchor)
    assert w_y.date_from == w_y.date_to == datetime.date(2026, 9, 10)
    anchor = datetime.date(2026, 8, 15)
...
def test_date_filters_from_request_prefers_rest_window():
    from gemini_brain.router.dates import date_filters_from_request

    out = date_filters_from_request(
        {"startDate": "2026-09-11", "endDate": "2026-09-11"},
        "What entries were posted today?",
        anchor=datetime.date(2026, 9, 11),
    )
    assert out == {"date_from": "2026-09-11", "date_to": "2026-09-11"}


def test_date_filters_from_request_finds_today_in_sentence():
    from gemini_brain.router.dates import date_filters_from_request
...
def test_date_filters_from_request_does_not_invent_a_year():
    from gemini_brain.router.dates import date_filters_from_request

    assert date_filters_from_request(None, "show journals", anchor=datetime.date(2026, 9, 11)) == {}
