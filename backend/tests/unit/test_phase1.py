...
def test_narration_failure_falls_back_to_short_notice():
    """Every answer is LLM-narrated; when Bedrock narration fails after
    retries, the answer falls back to a short line rather than repeating the
    full formatted table -- table_markdown/blocks already carry that data to
    the frontend separately, so duplicating it into the prose answer would
    show the same figures twice."""
    ...
    with patch("gemini_brain.orchestrator.gemini_brain_runner.classify_intent") as mock_classify, \
        ...
        # The answer is a short fallback line, not the retrieved table's
        # content -- table_markdown/blocks (asserted separately elsewhere)
        # already carry the figures to the frontend, so the prose answer
        # doesn't repeat them.
        assert res["answer"].startswith("Here's your")
        assert "Total Sales" not in res["answer"]
        assert mock_reason.call_count == 2  # retried once before falling back
