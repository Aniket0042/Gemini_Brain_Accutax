...
def test_journal_verifier_builder_filters_today():
    """'Posted today' must not verify emptiness against all-time journals."""
    task, builder = _empty_result_verifier_for("/accounting/journal-entries")
    assert task == "journal_entry_search"
    params = builder("What entries were posted today?", 2)
    assert params["organization_id"] == 2
    filters = params.get("filters") or {}
    from gemini_brain.router.dates import today as org_today
    day = org_today().isoformat()
    assert filters.get("date_from") == day
    assert filters.get("date_to") == day


def test_verify_empty_sql_keeps_rest_date_and_does_not_override_zero(monkeypatch):
    """REST empty for 11 Sep must stay empty when SQL for that same day is also empty.

    The previous bug dropped startDate/endDate, ran journal_entry_search with
    LIMIT 100, and replaced 'nothing posted today' with months of old journals.
    """
    captured = {}

    def fake_handler(task, params):
        captured["task"] = task
        captured["params"] = params
        return {"success": True, "results": []}

    monkeypatch.setattr(
        "gemini_brain.sql_fallback.sql_engine._get_coordinator_pipeline",
        lambda: (None, None, {"finance_agent": fake_handler}),
    )
    from gemini_brain.orchestrator.gemini_brain_runner import _verify_empty_via_sql

    out = _verify_empty_via_sql(
        "/accounting/journal-entries",
        2,
        "What entries were posted today?",
        {"startDate": "2026-09-11", "endDate": "2026-09-11"},
    )
    assert out is None
    assert captured["task"] == "journal_entry_search"
    assert captured["params"]["filters"]["date_from"] == "2026-09-11"
    assert captured["params"]["filters"]["date_to"] == "2026-09-11"


def test_verify_empty_sql_overrides_only_when_same_window_has_rows(monkeypatch):
    def fake_handler(task, params):
        assert params["filters"]["date_from"] == "2026-09-11"
        return {
            "success": True,
            "results": [{
                "journal_number": "JE-2-2026-00026099",
