...
def test_llm_router_injects_feedback_into_prompt():
    ...
    def dummy_gemini(system_prompt, user_message, max_tokens=250, thinking_budget=0, tools=None, **kwargs):
...
def test_empty_and_denied_do_not_trigger_self_correction():
    ...
    with patch("gemini_brain.orchestrator.gemini_brain_runner.fast_route", return_value=None), \
         ...
         patch("gemini_brain.orchestrator.gemini_brain_runner.organization_exists", return_value=True):
        # Both _verify_empty_via_sql and organization_exists reach a real DB
        # connection outside of the mocked runner._retrieve above, and either can
        # rewrite the EMPTY outcome this test asserts on -- organization_exists
        # reports "tenant not in this database" when org 27 is absent. Both are
        # unrelated to the self-correction gating under test, so both are forced
        # off here.
