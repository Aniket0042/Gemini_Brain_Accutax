...
def test_fast_router_income_chart_uses_profit_loss_api():
    """Graphs of income must hit the P&L report API, not the scalar SQL total."""
    res = fast_route("graph revenue this year", organization_id=27)
    assert res is not None
    assert res.endpoint == "/report/profit-loss"
    assert "start_date" in res.query_params
...
def test_fast_router_profit_loss_by_project():
    res = fast_route("least profitable projects", organization_id=27)
    assert res is not None
    assert res.endpoint == "/report/profit-loss-by-project"
    assert res.query_params.get("sort_order") == "asc"
...
def test_fast_router_cash_forecast():
    ...
    assert res.intent == 5
    # The horizon follows the question. This previously asserted 6 because the
    # month count was hardcoded, so "next month" and "next 12 months" both
    # produced an identical 6-month forecast.
    assert res.query_params["months"] == 1


def test_forecast_horizon_parsing():
    """The horizon parser itself, independent of which phrasings the rule matches."""
    from gemini_brain.router.dates import forecast_months

    assert forecast_months("cash forecast for the next 3 months") == 3
    assert forecast_months("cash flow projection for next 12 months") == 12
    assert forecast_months("expected cash position next quarter") == 3
    assert forecast_months("show me the cash forecast") == 6  # no horizon named
    assert forecast_months("forecast the next 500 months") == 36  # bounded


def test_fast_router_cash_forecast_honours_horizon():
    """Horizon reaches query_params for phrasings the cash_forecast rule matches."""
    for query, expected in [
        ("cash forecast for the next 3 months", 3),
        ("show me the cash forecast", 6),
    ]:
        res = fast_route(query, organization_id=27)
        assert res is not None, query
        assert res.query_params["months"] == expected, query
...
def test_fast_router_journal_entries_posted_today():
    res = fast_route("What entries were posted today?", organization_id=27)
    assert res is not None
    assert res.endpoint == "/accounting/journal-entries"
    from gemini_brain.router.dates import today as org_today
    day = org_today().isoformat()
    assert res.query_params["startDate"] == day
    assert res.query_params["endDate"] == day
    assert res.query_params["start_date"] == day
    assert res.query_params["organizationId"] == 27
