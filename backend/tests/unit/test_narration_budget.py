...
def test_brief_budget_overrides_shape():
    prompt, tokens = narration_budget("tabular", brief=True)
    assert prompt is ANALYST_SYSTEM_PROMPT_BRIEF
    assert tokens == min(NARRATION_MAX_TOKENS_SCALAR, 220)
...
def test_both_prompts_carry_every_accuracy_rule(rule):
    ...
    assert rule in ANALYST_SYSTEM_PROMPT_BRIEF, f"brief prompt lost: {rule}"
...
def test_prompts_contain_no_format_placeholders():
    ...
    """
    for name, prompt in (
        ("concise", ANALYST_SYSTEM_PROMPT),
        ("detailed", ANALYST_SYSTEM_PROMPT_DETAILED),
        ("brief", ANALYST_SYSTEM_PROMPT_BRIEF),
    ):
        assert "{" not in prompt and "}" not in prompt, f"{name} prompt contains a brace"


def test_concise_prompt_keeps_the_word_cap():
    assert "Maximum 80 words" in ANALYST_SYSTEM_PROMPT
...
def test_formatted_duplicate_fields_are_stripped_from_payload():
    system, user_msg = _format_payload_and_system(
        query="P&L",
        data={
            "operating_income": 1777028.25,
            "formatted_operating_income": "1777028.25 AED",
            "summary": {"net_profit": 1302990.85, "formatted_net_profit": "1302990.85 AED"},
        },
        endpoint="/report/profit-loss",
    )
    assert "formatted_operating_income" not in user_msg
    assert "formatted_net_profit" not in user_msg
    assert "1777028.25" in user_msg
    assert "Ignore any field named formatted_*" in system


def test_graph_series_are_stripped_from_narration_payload():
    _, user_msg = _format_payload_and_system(
        query="P&L",
        data={
            "operating_income": 100,
            "graphData": {"labels": ["Jan"], "incomeValues": [1], "expenseValues": [2]},
        },
        endpoint="/report/profit-loss",
    )
    assert "graphData" not in user_msg
    assert "incomeValues" not in user_msg
    assert "operating_income" in user_msg
