...
def test_every_registered_report_has_a_definition():
    ...
    for rest_ep, sql_key in defs.REST_TO_SQL_REPORT.items():
        ...
        assert any(spec.endpoint == rest_ep for spec in REGISTRY.values()), rest_ep
...
def test_report_tools_prefer_rest_with_sql_fallback():
    """Registered report tools call Nest /report/*; SQL rpt_ is fallback only."""
    rest_tools = {
        "aged_receivables_detail": "/report/invoice-details",
        "aged_payables_detail": "/report/aged-payables-detail",
        "bills_by_contact": "/report/bills-by-contact",
        "expenses_by_contact": "/report/expenses-by-contact",
        "supplier_statement": "/report/supplier-statement-of-account",
        "profit_loss_by_project": "/report/profit-loss-by-project",
        "profit_loss_by_cost_center": "/report/profit-loss-by-cost-center",
        "sales_by_project": "/report/sales-by-project",
        "estimate_conversion": "/report/estimate-conversion-rate",
        "vat_input_output": "/report/vat-input-output",
        "vat_export_return": "/report/export-return",
    }
    for name, endpoint in rest_tools.items():
        assert REGISTRY[name].endpoint == endpoint, name
        sql_key = defs.REST_TO_SQL_REPORT.get(endpoint)
        assert sql_key in defs.REPORTS, endpoint
