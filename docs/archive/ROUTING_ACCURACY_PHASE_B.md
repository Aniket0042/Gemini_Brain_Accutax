...
| **Overall Routing Accuracy** | **95.0%** (76/80) | Correct intent, endpoint/task, and parameter matching |
| **Layer 1 (Fast Router) Hit Rate** | **32.5%** (26/80) | Queries intercepted deterministically with 0 LLM calls |
| **Layer 1 Accuracy on Hits** | **88.5%** (23/26) | Precision of Layer 1 fast-router regex rules |
| **Left Path (Concept/Guidance) Accuracy** | **100.0%** (13/13) | Concept guard and guidance path routing |
| **Layer 1 Latency (p50 / p95)** | **0.04ms / 0.09ms** | Sub-millisecond deterministic evaluation |
| **Overall Routing Latency (p50 / p95)** | **0.07ms / 0.17ms** | End-to-end routing decision time |
...
|---|---|---|---|---|---|
| `canonical` | 33 | 31 | **93.9%** | 23 | High precision via deterministic rules |
...
| `synonym` | 23 | 23 | **100.0%** | 1 | Phrasings not covered in hand-coded regexes |
...
| Q23 | Show general ledger profitability by acc | `layer1_fast` | `layer3_sql_loop` | `fn_gl_profitability` | `fn_gl_profitability` | ✅ |
| Q24 | How much money did our business make in  | `layer2_llm_api` | `layer2_llm_api` | `/income/total` | `/income/total` | ✅ |
| Q25 | What are our overall expenditures for th | `layer2_llm_api` | `layer2_llm_api` | `/expense/total` | `/expense/total` | ✅ |
